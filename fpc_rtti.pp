...
{$endif}
  ...
  public
    ...
    function GetFields:specialize  TArray<TRttiField>; virtual;
    function GetDeclaredMethods: specialize TArray<TRttiMethod>; virtual;
    function GetDeclaredFields:specialize  TArray<TRttiField>;
...
type
  TRttiFieldArrays =  array of array of TRttiField;//specialize TArray<T>;
  TArrayHelper = class
  public
    class function Concat(constref Args: TRttiFieldArrays): specialize TArray<TRttiField>; static;
  end;

  class function TArrayHelper.Concat(constref Args: TRttiFieldArrays):specialize TArray<TRttiField>;
var
  i, j, out, len: Integer;
begin
  len := 0;
...
function TRttiType.GetDeclaredFields:specialize  TArray<TRttiField>;
begin
  Exit(nil);
end;

function TRttiType.GetFields: specialize  TArray<TRttiField>;
var
  flat: array of array of TRttiField;// TArray<TArray<TRttiField>>;
  t: TRttiType;
  depth: Integer;
begin
  t := Self;
  ...
  Result := TArrayHelper.Concat(flat);
end;

function InsufficientRtti: Exception;
begin
  Result := EInsufficientRtti.Create(SInsufficientRtti);
end;
...
begin
  ...
  Result := fpc_Rtti.Invoke(Name, addr, CallingConvention, IsStatic, aInstance, aArgs, GetParameters(True), ReturnType);
end;
...
begin
  ...
  Result := fpc_Rtti.Invoke(Name, method^.Code, CallingConvention, False, inst, aArgs, GetParameters(True), ReturnType);
end;
...
begin
  ...
  Result := fpc_Rtti.Invoke(Name, PCodePointer(aCallable.GetReferenceToRawData)^, CallingConvention, True, TValue.Empty, aArgs, GetParameters(True), ReturnType);
end;
...
begin
  result := fpc_Rtti.IsManaged(FTypeInfo);
end;
