unit openssl3.crypto.rsa.rsa_sign;
{$I config.inc}
interface
 uses OpenSSL.Api;
...
uses OpenSSL3.Err, openssl3.crypto.mem, OpenSSL3.crypto.rsa.rsa_crpt;
...
{$endif} { FIPS_MODULE }
    ...
    ret := 1;
_err:
    OPENSSL_clear_free(Pointer(encoded), encoded_len);
    OPENSSL_clear_free(Pointer(decrypt_buf), siglen);
    Result := ret;
end;
...
{$endif} { FIPS_MODULE }
    ...
    ret := 1;
_err:
    OPENSSL_clear_free(Pointer(tmps), encoded_len);
    Result := ret;
end;
