...
function test_rsa_simple( idx, en_pad_type, de_pad_type, success : integer;
                          ctext_ex : PByte; clen : PInteger; retkey : PPRSA):integer;
var
  ret      : integer;
  key      : PRSA;
  ptext,  ctext    : array[0..255] of Byte;
  plen,
  clentmp,
  num      : integer;
...
begin
    ...
    if 0>=TEST_int_eq('num', 'clentmp', num, clentmp) then
        goto _err;
    num := RSA_private_decrypt(num, @ctext[0]{from}, @ptext[0]{to}, key, de_pad_type);
    if success > 0 then
    begin
...
begin
    ...
    b := RSA_set0_key(key, BN_bin2bn(@num, _bytes, nil) , BN_bin2bn(@num, _bytes, nil), nil);
    if (TEST_true('RSA_set0_key(key, BN_bin2bn(num, bytes, NULL), BN_bin2bn(num, bytes, NULL), NULL)', b) > 0)
...
begin
    RSA_set0_key(key, BN_bin2bn(@n, sizeof(n), nil),
                     BN_bin2bn(@e, sizeof(e), nil),
                     BN_bin2bn(@d, sizeof(d), nil));
    RSA_set0_factors(key, BN_bin2bn(@p, sizeof(p), nil),
                     BN_bin2bn(@q, sizeof(q), nil));
    RSA_set0_crt_params(key, BN_bin2bn(@dmp1, sizeof(dmp1), nil),
                        BN_bin2bn(@dmq1, sizeof(dmq1), nil),
                        BN_bin2bn(@iqmp, sizeof(iqmp), nil));
    if c <> nil then
       memcpy(c, @ctext_ex, sizeof(ctext_ex));
    Result := sizeof(ctext_ex) ;
...
begin
    ...
    RSA_set0_key(key,
                     BN_bin2bn(@n, sizeof(n), nil),
                     BN_bin2bn(@e, sizeof(e), nil),
                     BN_bin2bn(@d, sizeof(d), nil));
    RSA_set0_factors(key,
                     BN_bin2bn(@p, sizeof(p), nil),
                     BN_bin2bn(@q, sizeof(q), nil));
    RSA_set0_crt_params(key,
                        BN_bin2bn(@dmp1, sizeof(dmp1), nil),
                        BN_bin2bn(@dmq1, sizeof(dmq1), nil),
                        BN_bin2bn(@iqmp, sizeof(iqmp), nil));
    if c <> nil then
       memcpy(c, @ctext_ex, sizeof(ctext_ex));
